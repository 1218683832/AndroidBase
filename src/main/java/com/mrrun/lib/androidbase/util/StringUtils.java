...
public class StringUtils {
	...
	public static double toDouble(String doubleStr) {
		double ret = 0;
		if (LocalTextUtil.isNoBlank(doubleStr)) {
			try {
	...
	public static float toFloat(String floatStr) {
		float ret = 0;
		if (LocalTextUtil.isNoBlank(floatStr)) {
			try {
	...
	public static String[] split(String string, char ch) {
		...
		if (nextStart < chars.length) { // 如果最后一位不是分隔符的话，就将最后一个分割符到最后一个字符中间的左右字符串作为一个字符串添加到分割数组中去
			stringList.add(new String(chars, nextStart, chars.length - 1
					- nextStart + 1));
		}
	...
	public static String toLowerCase(String str, int beginIndex, int endIndex) {
		return str.replaceFirst(
				...
				str.substring(beginIndex, endIndex).toLowerCase(
						Locale.getDefault()));
	}
	...
	public static String toUpperCase(String str, int beginIndex, int endIndex) {
		return str.replaceFirst(
				str.substring(beginIndex, endIndex),
				str.substring(beginIndex, endIndex).toUpperCase(
						Locale.getDefault()));
	}
	 ...
	 * @return
	 */
	public static boolean startsWithIgnoreCase(String sourceString,
			String newString) {
	 ...
	 * @return
	 */
	public static boolean endsWithIgnoreCase(String sourceString,
			String newString) {
		int newLength = newString.length();
		int sourceLength = sourceString.length();
		if (newLength == sourceLength) {
